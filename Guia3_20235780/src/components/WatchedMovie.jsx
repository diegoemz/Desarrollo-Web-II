...
export function WatchedMoviesList({ watched, removeWatchedMovies }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie movie={movie} key={movie.imdbID} removeWatchedMovies={removeWatchedMovies}/>
      ))}
    </ul>
  );
}
export function WatchedMovie({ movie, removeWatchedMovies }) {
  return (
    <li>
      ...
      <div>
        ...
        <button className="btn-delete" onClick={()=>removeWatchedMovies(movie.imdbID)}>X</button>
      </div>
    </li>
  );
}
